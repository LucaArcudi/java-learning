...
3. **Consegna:** quando dichiaro conclusa l'implementazione, ispeziona il mio codice e, quando utile, i commit che ne mostrano l'evoluzione; esegui compilazione e programma e prepara la spiegazione della soluzione.
4. **Spiegazione:** crea `review.md` allo stesso livello di `trace.md`. Parti dalla mia implementazione per spiegare cosa funziona, quali scelte sono diventate artificiali e come le responsabilità potrebbero essere modellate in modo più naturale. La review deve insegnare come ragionare sulla soluzione, non limitarsi a elencare difetti o imporre un ciclo di correzioni. Non modificare il mio codice e non scrivere ancora la soluzione AI.
5. **Correzioni facoltative:** dopo la spiegazione posso scegliere se modificare il codice, ma la riscrittura non è obbligatoria per proseguire, salvo che sia io a richiederla. Non creare ancora `aireview/`.
6. **Domande finali:** dopo la spiegazione, formula domande basate sulla mia implementazione reale, sulla review e sugli argomenti del blocco. Non dare per scontate classi, relazioni o scelte che non ho effettuato; eventuali casi diversi dal mio codice devono essere presentati esplicitamente come ipotesi. Salva le domande in `questions.md`, allo stesso livello di `trace.md`.
7. **Risposte finali:** scrivo le mie risposte in `solution/answers.md`. Valutale senza creare o anticipare le tue risposte.
8. **Chiusura:** dopo che le risposte finali sono state approvate, considera concluso l'esercizio, crea la tua implementazione separata dentro `aireview/` come termine di paragone e scrivi le tue risposte in `aireview/answers.md`. Infine aggiorna `ROADMAP.md`.
